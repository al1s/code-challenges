...
const grandTotal = (hours, stores) => {
  ...
  for (let i = 0; i < hoursOpen.length; i++) {
    arrResult.push(stores.reduce((sum, store) => (sum += store[i]), 0));
  }
  return arrResult;
};
...
const salesData = data => {
  // Solution code here...
  let arrResult = [];
  data.forEach((elm, ndx) =>
    arrResult.push({ sales: `${elm} cookies`, time: hoursOpen[ndx] })
  );
  return arrResult;
};
...
describe("Testing challenge 1", () => {
  test("It should add the hourly totals array", () => {
    expect(grandTotal(hoursOpen, cookieStores)).toStrictEqual([
      88,
      153,
      252,
      286,
...
describe("Testing challenge 2", () => {
  test("It should create an object of data for each store", () => {
    ...
    expect(salesData(grandTotal(hoursOpen, cookieStores)).length).toStrictEqual(
      hoursOpen.length
    );
  });
});
