...
const averageDailyTemperature = weather => {
  ...
  let result = 0;
  let weatherFlatten = [];
  weather.forEach(elm => (weatherFlatten = [...weatherFlatten, ...elm]));
  weatherFlatten.forEach(elm => (result += elm));
  return result / weatherFlatten.length;
};
...
const lowestWeeklyAverage = weather => {
  ...
  weather.forEach(elm => {
    let weeklyAvg = 0;
    elm.forEach(elmInner => (weeklyAvg += elmInner));
    weeklyAvg = Math.round(weeklyAvg / elm.length);
    result = result || weeklyAvg;
    result = result < weeklyAvg ? result : weeklyAvg;
  });
  return result;
};
